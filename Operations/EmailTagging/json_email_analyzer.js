...
class EmailCampaignAnalyzer {
    constructor() {
        ...
        // UPDATED: Role Level Detection (4 levels) - Now detects from content themes
        this.roleLevelKeywords = {
            'C-Suite': [
                'ceo', 'cto', 'cfo', 'coo', 'chief', 'president', 'founder',
                'board', 'executive', 'leadership', 'vision', 'strategy',
                'company direction', 'stakeholder', 'investor', 'board meeting',
                'company culture', 'market positioning', 'competitive advantage'
            ],
            'VP-Director': [
                'vp', 'vice president', 'director', 'head of', 'lead',
                'department', 'budget', 'team management', 'hiring',
                'resource allocation', 'cross-functional', 'roadmap',
                'quarterly goals', 'performance metrics', 'team building'
            ],
            'Manager': [
                'manager', 'team lead', 'supervisor', 'coordinator',
                'sprint', 'daily standup', 'project management', 'deadline',
                'team productivity', 'workflow', 'process improvement',
                'team communication', 'task assignment', 'progress tracking'
            ],
            'Individual': [
                'engineer', 'developer', 'analyst', 'specialist', 'consultant',
                'coding', 'implementation', 'debugging', 'technical', 'hands-on',
                'development', 'programming', 'analysis', 'execution',
                'technical skills', 'tools', 'frameworks', 'best practices'
            ]
        };

        // UPDATED: Function Area Detection (expanded to match subscriber form)
        this.functionAreaKeywords = {
            'Engineering': [
                'engineering', 'development', 'technical', 'code', 'software',
                'infrastructure', 'devops', 'architecture', 'api', 'developer',
                'programming', 'coding', 'debugging', 'deployment', 'testing'
            ],
            'Product Management': [
                'product', 'pm', 'product manager', 'roadmap', 'feature', 'backlog',
                'user story', 'requirements', 'stakeholder', 'prioritization',
                'product strategy', 'user experience', 'market research'
            ],
            'Design': [
                'design', 'ui', 'ux', 'user interface', 'user experience', 'visual',
                'creative', 'designer', 'prototype', 'wireframe', 'mockup',
                'branding', 'graphics', 'layout', 'typography', 'aesthetic'
            ],
            'Content': [
                'content', 'writing', 'copywriting', 'marketing', 'blog', 'social',
                'content strategy', 'editorial', 'messaging', 'communication',
                'content creation', 'storytelling', 'brand voice'
            ],
            'Leadership': [
                'leadership', 'executive', 'strategy', 'vision', 'planning',
                'business strategy', 'decision making', 'team management',
                'organizational', 'strategic planning', 'company direction'
            ],
            'Operations': [
                'operations', 'ops', 'process', 'workflow', 'project management',
                'business operations', 'efficiency', 'optimization', 'systems'
            ],
            'Growth': [
                'growth', 'marketing', 'sales', 'acquisition', 'conversion',
                'revenue', 'business development', 'customer acquisition',
                'growth hacking', 'metrics', 'analytics'
            ],
            'Support': [
                'support', 'customer success', 'help', 'service', 'training',
                'onboarding', 'documentation', 'customer service'
            ]
        };

        // UPDATED: Industry Detection (refined for business size distinctions)
        this.industryKeywords = {
            'Startup': [
                'startup', 'early stage', 'seed funding', 'pre-seed', 'bootstrap',
                'mvp', 'product-market fit', 'founder', 'co-founder', 'small team',
                'series a', 'pivot', 'runway', 'burn rate', '5 employees', '10 employees'
            ],
            'SMB': [
                'small business', 'smb', 'small-medium business', 'family business',
                'local business', 'independent', 'sole proprietor', 'small company',
                '20 employees', '50 employees', 'tight budget', 'limited resources'
            ],
            'Mid-Market': [
                'mid-market', 'growing company', 'scale-up', 'expanding business',
                'regional company', 'established business', '100 employees', '500 employees',
                'multiple locations', 'department heads', 'structured teams'
            ],
            'Enterprise': [
                'enterprise', 'large corporation', 'multinational', 'fortune 500',
                'global company', 'big corporation', '1000 employees', 'enterprise-grade',
                'complex organization', 'multiple divisions'
            ],
            'Technology': [
                'tech company', 'software company', 'saas', 'platform', 'digital product',
                'app company', 'technology startup', 'dev team', 'engineering team'
            ],
            'Services': [
                'consulting', 'agency', 'professional services', 'service provider',
                'advisory firm', 'consulting firm', 'creative agency'
            ]
        };
    ...
    async analyzeEmail(htmlContent, emailId = null, subscriberData = null) {
        ...
        if (subscriberData) {
            ...
            // Detect from content and suggest likely interested role levels
            ...
            // Add suggested target roles based on content analysis
            analysis.suggested_target_roles = this.suggestTargetRoles(lowerText, analysis.campaign_tags);
        }
    ...
    /**
     * NEW: Suggest target roles based on content analysis
     */
    suggestTargetRoles(text, campaignTags) {
        const suggestions = [];
        
        // Strategic content → C-Suite + VP-Director
        if (/strategy|vision|market|competitive|growth|scale|expansion|investment/i.test(text)) {
            suggestions.push('C-Suite', 'VP-Director');
        }
        
        // Technical content → Individual + Manager
        if (/code|api|development|technical|implementation|bug|system/i.test(text)) {
            suggestions.push('Individual', 'Manager');
        }
        
        // Process/workflow content → Manager + VP-Director  
        if (/process|workflow|team|management|efficiency|productivity/i.test(text)) {
            suggestions.push('Manager', 'VP-Director');
        }
        
        // Budget/cost content → VP-Director + C-Suite
        if (/budget|cost|roi|revenue|savings|investment|pricing/i.test(text)) {
            suggestions.push('VP-Director', 'C-Suite');
        }
        
        // Innovation/AI content → All levels (different angles)
        if (campaignTags.content_interests?.includes('AI-ML')) {
            return {
                'C-Suite': 'Strategic AI adoption and competitive positioning',
                'VP-Director': 'AI implementation planning and team coordination', 
                'Manager': 'AI workflow integration and team training',
                'Individual': 'Hands-on AI tools and technical implementation'
            };
        }
        
        // Return unique suggestions with targeting rationale
        const uniqueSuggestions = [...new Set(suggestions)];
        const rationale = {};
        
        uniqueSuggestions.forEach(role => {
            if (role === 'C-Suite') rationale[role] = 'Strategic decision-making content';
            if (role === 'VP-Director') rationale[role] = 'Planning and coordination themes';
            if (role === 'Manager') rationale[role] = 'Team and process management focus';
            if (role === 'Individual') rationale[role] = 'Technical implementation details';
        });
    ...
    mapSubscriberFunction(formRole) {
        ...
        return functionMapping[formRole] || 'Engineering';
    }
